...
<div class="row">
    <div class="col-md-12">
        <div style="padding-bottom: 10px;">
            <a href="#tambahproduk" role="button" class="btn btn-primary" data-toggle="modal">Tambah Slider</a>
        </div>

        <div id="tambahproduk" class="modal fade" tabindex="-1">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form class="form-horizontal" role="form" id="formAddSlider" action="<?= base_url(uri_string() . '/add_produk') ?>" method="POST" enctype="multipart/form-data">
                        <input type="reset" class="hidden">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                            <h3 class="smaller lighter blue no-margin">Tambah Slider</h3>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-12">
                                    <label>Gambar Slider</label>
                                    <input type="file" class="form-control" name="nama_slider" id="input_foto5" required>
                                </div>
                            </div>
                        </div>
                    ...
                    foreach ($result as $key => $value) { ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <?php if ($value->nama_slider) { ?>
                                    <img src="<?= base_url('assets/img/' . $value->nama_slider) ?>" width="100">
                                <?php } ?>
                            </td>
                            <td>
                                <button type="button" class="btn btn-primary btn-xs" onclick="edit_produk('<?= $value->id_slider ?>')">
                                    <i class="fa fa-edit"></i>
                                </button>
                                <button type="button" class="btn btn-danger btn-xs" onclick="delete_produk('<?= $value->id_slider ?>')">
